...
class NeuralNet:
    ...
    def prune(self, threshold=0):
        """Removes any wires in the net with weight less than threshold,
           then removes any nodes with no outgoing wires
           
           Input:
                theshold (float): a magnitude of weight, any nodes with weight of magnitude less
                                  than threshold are removed
           Returns: 
                (int, int) the number of wires removed and the number of neurons removed
        """
        wires_removed = 0
        neurons_removed = 0

        for wire in self.wires[:]:
            if abs(wire.weight) < threshold:
                self.wires.remove(wire)
                wires_removed += 1

        for neuron in self.neurons[:]:
            if len(self.get_outgoing_wires[neuron]):
                self.neurons.remove(neuron)
                neurons_removed += 1
        return (wires_removed, neurons_removed)
